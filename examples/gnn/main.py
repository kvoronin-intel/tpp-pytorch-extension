...
class SAGE(nn.Module):
    def __init__(self,
        ...
        self.layers.append(dgl.nn.pytorch.conv.SAGEConv(in_feats, n_hidden, 'mean', feat_drop=dropout, activation=activation))
        for i in range(1, n_layers - 1):
            self.layers.append(dgl.nn.pytorch.conv.SAGEConv(n_hidden, n_hidden, 'mean', feat_drop=dropout, activation=activation))
    ...
    def inference(self, g, x, device):
        ...
        for l, layer in enumerate(self.layers):
            if args.use_bf16:
              y = th.zeros(g.num_nodes(), self.n_hidden if l != len(self.layers) - 1 else self.n_classes).to(device).to(th.bfloat16)
            else:
...
def run(args, device, data):
    ...
    if args.use_pcl:
        ...
        optimizer = ppx.optim.AdamW(optimizer_grouped_parameters, lr=args.lr, eps=args.adam_epsilon)
    else:
    ...
    best_eval_acc = 0
    best_test_acc = 0
    if args.use_pcl: ppx.manual_seed(args.seed)
    record_shapes = False
    with th.autograd.profiler.profile(enabled=args.profile, use_cuda=(args.gpu > 0), record_shapes=record_shapes) as prof:
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--use_pcl', action="store_true", help="Whether to use PCL Fused impl when available")
    argparser.add_argument('--use_bf16', action="store_true", help="Whether to use PCL Fused impl when available")
    ...
    argparser.add_argument("--profile", action="store_true", help="Whether to use PCL Fused impl when available")
