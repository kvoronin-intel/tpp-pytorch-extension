...
def process_dataset(
    ...
    train_idx = torch.nonzero(
        g.nodes[predict_category].data["train_mask"], as_tuple=True
    )[0]
    valid_idx = torch.nonzero(
        g.nodes[predict_category].data["valid_mask"], as_tuple=True
    )[0]
    test_idx = torch.nonzero(
        g.nodes[predict_category].data["test_mask"], as_tuple=True
    )[0]
