        ...
        for ntype in hg.ntypes:
            if node_feats[ntype] is None:
                ...
                embedding = nn.Parameter(
                    torch.Tensor(input_embedding_size, embedding_size)
                )
...
class RelGraphConvLayer(nn.Module):
    def __init__(
        ...
        self._conv = dglnn.HeteroGraphConv(
            {
                rel: dglnn.GraphConv(
                    in_feats, out_feats, norm=norm, weight=False, bias=False
                )
        ...
        if self_loop:
            self.self_loop_weight = nn.Parameter(torch.Tensor(in_feats, out_feats))
            nn.init.xavier_uniform_(
    ...
    def forward(
        ...
        if self._use_weight:
            ...
            weight_dict = {
                self._rel_names[i]: {"weight": w.squeeze(dim=0)}
        ...
        if self._use_self_loop:
            if hg.is_block:
                inputs_dst = {
                    ntype: h[: hg.num_dst_nodes(ntype)] for ntype, h in inputs.items()
                }
            else:
                inputs_dst = inputs
        else:
        ...
        x = {ntype: self._apply_layers(ntype, h, inputs_dst) for ntype, h in x.items()}
    ...
    def inference(
        ...
        for i, layer in enumerate(self._layers):
            ...
            if i < self._num_layers - 1:
                y = {
                    ntype: torch.zeros(
                        hg.num_nodes(ntype), self._hidden_feats, device=device
                    )
                    for ntype in hg.ntypes
                }
            else:
                y = {
                    ntype: torch.zeros(
                        hg.num_nodes(ntype), self._out_feats, device=device
                    )
                    for ntype in hg.ntypes
                }
            ...
            for in_nodes, out_nodes, blocks in dataloader:
                ...
                if i == 0:
                    ...
                    h = {ntype: x[ntype][in_nodes[ntype]] for ntype in hg.ntypes}
