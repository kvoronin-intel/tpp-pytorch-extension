...
class Net2(nn.Module):  # Second network containing our optimized layer
    ...
    def forward(self, x):
        x = self.padding_layer(
            x
        )  # Explicit padding needed for our optimzed convolutional layer
        x = self.conv2(x)
...
X = torch.randn(
    Batch_size, Channels, Input_width, requires_grad=True
...
print(
    "Backward weight check: ",
    (
        (torch.abs(wgrad1 - wgrad2) / r < 0.01).sum()
...
print(
    "    Foward pass check: ",
    (
        (torch.abs(Y1 - Y2) / r < 0.01).sum() == Batch_size * Filters * Input_width
    ).item(),
)
...
print(
    "  Backward data check: ",
    (
        (torch.abs(dgrad1[0] - dgrad2[0]) / r < 0.01).sum()
