...
X = torch.randn(
    Batch_size_1, Channels, Input_width, requires_grad=True
)  # Random Input (Batch_size, channel, width)
...
X = torch.randn(
    Batch_size_2, Channels, Input_width, requires_grad=True
)  # Random Input (Batch_size, channel, width)
...
print(
    "Backward pass time (PyTorch layer): {:.3f} ms | Backward pass time (Optimized layer): {:.3f} ms".format(
        backward1 * 1e3 / N, backward2 * 1e3 / N
    )
)
...
forward1_flops = (
    ...
    * Filters
    * Kernel_size
    * (Input_width - (Kernel_size - 1) * Dilation)
    / (forward1 / N)
)
backward1_flops = (
    2
...
print("\n")
print(
    "Forward pass flops (PyTorch layer): {:e} | Forward pass flops (Optimized layer): {:e} ".format(
        forward1_flops, forward2_flops
    )
)
print(
    "Backward pass flops (PyTorch layer): {:e} | Backward pass flops (Optimized layer): {:e} ".format(
        backward1_flops, backward2_flops
    )
)
