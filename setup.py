...
#    url="https://github.com/pypa/sampleproject",
    ...
    #ext_modules=[CppExtension('pcl_pytorch_extension._C', glob.glob('src/csrc/*.cpp') + glob.glob('src/csrc/bert/*.cpp') + glob.glob('src/csrc/gnn/*.cpp'), extra_compile_args=['-fopenmp', '-g', '-march=native'],
    ext_modules=[CppExtension('pcl_pytorch_extension._C', glob.glob('src/csrc/*.cpp') + glob.glob('src/csrc/gnn/*/*.cpp'), extra_compile_args=['-fopenmp', '-g', '-march=native'],
      include_dirs=['{}/include/'.format(os.getenv("LIBXSMM_ROOT")), '{}/src/csrc'.format(os.getenv("PWD"))],
