    ...
    // DECL_VLA_PTR_PT(T, d_weight_a, [C_t][WW_t], d_weight);
    ...
    //     for(int j = 0; j < C_t; j++){
    //         for(int kw = 0; kw < WW_t; kw++){                                   /* reverse copy */
    //             flip_weight_a[i][j][kw] = weight_a[i][j][WW_t - kw - 1];
    //         }
    //     }
    // }
    ...
    auto virtual_copy_zero = SCOPEIT(SetZeroTPP<T>(virtual_n, virtual_m2, ldo_virtual), EW_COPY);
...
*/
    ...
    auto copy_kernel_main = SCOPEIT(SetZeroTPP<T>(tpp_n, tpp_m1, ld_zero), EW_COPY);
    auto copy_kernel_edge = SCOPEIT(SetZeroTPP<T>(tpp_n, tpp_m2, ld_zero), EW_COPY);
    ...
    for(int n = 0; n < N_t; n++) {
        ...
        for(int wb = 0; wb < W_t - XS_TILE_WBACKWARD + 1; wb += XS_TILE_WBACKWARD) {     /* Normal case */
            ...
            trans_param_short.in.primary  = &grad_a[n][0][wb];
            ...
            for(int kw = 0; kw < WW_t; kw++) {
                ...
                gemm_param_main.b.primary = &input_a[n][0][wb + kw*dial];
        ...
        if (W_t % XS_TILE_WBACKWARD != 0){
            ...
            trans_param_edge.in.primary  = &grad_a[n][0][last_block + XS_TILE_WBACKWARD];
            ...
            for(int kw = 0; kw < WW_t; kw++) {
                ...
                gemm_param_edge.b.primary = &input_a[n][0][(last_block + XS_TILE_WBACKWARD) + kw*dial];
