...
int tile_multiple = (W_t / XS_TILE_FORWARD) *
...
auto main_brgemm_tpp = SCOPEITGEMM(
    (BrgemmTPP<
        T,
        T>(tpp_n, tpp_m1, tpp_k, F_t* C_t, dial, lda, ldb, ldc, 1.0, 0, 1)));
auto edge_brgemm_tpp = SCOPEITGEMM(
    (BrgemmTPP<
        T,
        T>(tpp_n, tpp_m2, tpp_k, F_t* C_t, dial, lda, ldb, ldc, 1.0, 0, 1)));
...
#pragma omp parallel for
    for (int n = 0; n < N_t; n++) { /* Loop for batches */
      ...
      for (int wb = 0; wb < W_t - XS_TILE_FORWARD + 1;
        ...
        main_brgemm_tpp(
            &flip_weight_a[0][0][0], &input_a[n][0][wb], &Y_a[n][0][wb], l_br);
...
// at::Tensor Conv1dOpti_forward_libxsmm(at::Tensor& input, at::Tensor& weight,
// int dilation){

//     /* RECORD_FUNCTION("Conv1dOpti_forward_libxsmm",
//     std::vector<c10::IValue>({input, weight}));    // For recording time   */
...
//     filters */ int64_t WW_t = weight.size(2); /* filter width */
...
//     auto Y = input.new_empty({N_t,F_t,W_t});                     /* New
//     tensor for output */

//     float* input_a = input.data_ptr<float>();                    /* Get
//     pointers for accessing the tensors */ float* weight_a =
//     weight.data_ptr<float>(); float* Y_a = Y.data_ptr<float>();

//     auto flip_weight = weight.new_empty({WW_t,F_t,C_t});        /* Array to
//     store permuted weight tensor (width, filters, channels) */ float*
//     flip_weight_a = flip_weight.data_ptr<float>();
...
//     per_m, per_n, per_ldi, per_ldo, LIBXSMM_DATATYPE_F32,
//     LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32 ); libxsmm_meltwfunction_unary
//     trans_permute_kernel = libxsmm_dispatch_meltw_unary_v2(
//     LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_NORM_TO_NORMT, unary_shape,
//     LIBXSMM_MELTW_FLAG_UNARY_NONE ); if ( trans_permute_kernel == NULL) {
//         fprintf( stderr, "JIT unary TPP for trans_permute_kernel (normal to
//         normal transform) isn't working in the forward pass. Bailing...!\n");
//         exit(-1);
//     }
...
//     int lda = C_t;                                              /* Input
//     channels (15) */ int ldb = Win_t; /* Input width (60400) */ int ldc =
//     W_t;                                              /* Output width
//     (60000)*/ unsigned long long l_br = WW_t;
...
//     l_shape = libxsmm_create_gemm_shape(XS_TILE_FORWARD, F_t, C_t, ldb, lda,
//     ldc, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32,
//     LIBXSMM_DATATYPE_F32); l_brconfig.br_type =
//     LIBXSMM_GEMM_BATCH_REDUCE_STRIDE; l_brconfig.br_stride_a_hint =
//     dial*sizeof(float); l_brconfig.br_stride_b_hint = F_t*C_t*sizeof(float);
//     libxsmm_gemmfunction brgemm_kernel_main =
//     libxsmm_dispatch_brgemm_v2(l_shape, l_flags, l_prefetch, l_brconfig);

//     l_shape = libxsmm_create_gemm_shape(W_t - tile_multiple, F_t, C_t, ldb,
//     lda, ldc, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32,
//     LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32); l_brconfig.br_type =
//     LIBXSMM_GEMM_BATCH_REDUCE_STRIDE; l_brconfig.br_stride_a_hint =
//     dial*sizeof(float); l_brconfig.br_stride_b_hint = F_t*C_t*sizeof(float);
//     libxsmm_gemmfunction brgemm_kernel_edge =
//     libxsmm_dispatch_brgemm_v2(l_shape, l_flags, l_prefetch, l_brconfig);
...
//     ); libxsmm_meltwfunction_unary zero_kernel_main =
//     libxsmm_dispatch_meltw_unary_v2( unary_type, unary_shape, unary_flags );
//     unary_shape = libxsmm_create_meltw_unary_shape( tpp_m2, tpp_n, tpp_m2,
//     ld_zero, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32
//     ); libxsmm_meltwfunction_unary zero_kernel_edge =
//     libxsmm_dispatch_meltw_unary_v2( unary_type, unary_shape, unary_flags );

//     if ((zero_kernel_main == NULL) || (zero_kernel_edge == NULL)) {
//         fprintf( stderr, "JIT UNARY kernel for initilizing zeros failed in
//         forward pass. Bailing...!\n"); exit(-1);
//     }

//     /* Main compute loop */
//     #pragma omp parallel for
//     for(int n = 0; n < N_t; n++) {                               /* Loop for
//     batches */
//         int last_block = 0;
...
//             zero_param_main.out.primary = &Y_a[n*F_t*W_t + wb];       /*
//             Initialization */ zero_kernel_main( &zero_param_main );
...
//             zero_param_edge.out.primary = &Y_a[n*F_t*W_t + last_block +
//             XS_TILE_FORWARD];     /* Initialization */ zero_kernel_edge(
//             &zero_param_edge );

//             gemm_param_edge.a.primary = &input_a[n*C_t*Win_t + 0*Win_t +
//             last_block + XS_TILE_FORWARD]; gemm_param_edge.b.primary =
//             &flip_weight_a[0]; gemm_param_edge.c.primary = &Y_a[n*F_t*W_t +
//             0*W_t + last_block + XS_TILE_FORWARD];
