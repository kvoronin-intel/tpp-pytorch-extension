...
if (input_trans_flag == XformTPP::XFORM_NONE_TPP) {
  ...
  auto trans_tpp = SCOPEIT(XformExtTPP<bfloat16>(bn, bc, XformTPP::XFORM_XPOSE_TPP), XPOSE);
  {
...
if(res) {
  if (input_trans_flag == XformTPP::XFORM_NONE_TPP) {
    ...
    auto trans_tpp = SCOPEIT(XformExtTPP<bfloat16>(bnp, bc, XformTPP::XFORM_XPOSE_TPP), XPOSE);
    {
      RECORD_FUNCTION("parallel_for", std::vector<c10::IValue>());
#pragma omp parallel for
      for (int n = 0; n < nn*nc; n++) {
...
auto n2v_tpp = SCOPEIT(XformExtTPP<T>(bn, bk, bnp, bk, XformTPP::XFORM_N2V_TPP, true), VNNI);
auto n2v_wt_tpp = SCOPEIT(XformExtTPP<T>(bc, bk, bcp, bk, XformTPP::XFORM_N2V_TPP, true), VNNI);
...
auto brgemm_di_tpp = SCOPEITGEMM(
    (BrgemmExtTPP<T, T>(bn, bc, bkp, bn*bkp, nc*bc*bkp, 0.0, XformTPP::XFORM_NONE_TPP, 0, nk)));
...
#pragma omp parallel for collapse(2)
    for(int n=0; n<nn; n++) {
      for(int c=0; c<nc; c++) {
        ...
        if(res) {
          if(bk != bkp)
            brgemm_di_tpp(grad_out_K[n][0], wt_res_TV[0][c], grad_in_res[n][c], nk);
          else
            brgemm_di_tpp(grad_out[n][0], wt_res_TV[0][c], grad_in_res[n][c], nk);
        }
      }
    }
  }
