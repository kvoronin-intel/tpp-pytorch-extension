...
typedef void (*par_loop_kernel)(LoopSpecs *loopSpecs, std::function<void(int*)>, std::function<void()>, std::function<void()>);
...
class LoopingScheme {
 public:
  LoopingScheme(std::string scheme) : scheme(scheme), nLogicalLoops(0), nLoops(0), barrierAfter(0), ompforBefore(-1), nCollapsed(0), nLLBL{0}, test_kernel(NULL) {
    int curLoop = 0;
    for (int i = 0; i < (int)scheme.length() - 1; i++) {
      ...
      if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
        ...
        printf("LoopingScheme: '%s': Ignoring unknown scheme character: '%c' at position %d\n", scheme.c_str(), scheme[i], i);
      }
    }
    nLoops = curLoop;
    for (int i = 0; i < nLoops; i++) {
      ...
      if (nLogicalLoops <= ll) nLogicalLoops = ll + 1;
      nLLBL[ll]++;
    }
    ...
    test_kernel = (par_loop_kernel)jit_from_str(code_str + gen_code, " -fopenmp ", "par_nested_loops");
  }

  void call(LoopSpecs *loopSpecs, std::function<void(int*)> body_func, std::function<void()> init_func, std::function<void()> fini_func) {
...
class ThreadedLoop {
 public:
  ThreadedLoop(std::array<LoopSpecs, N> bounds, std::string scheme = "") : bounds(bounds), scheme(scheme) {
    if (scheme == "") scheme = getDefaultScheme();
