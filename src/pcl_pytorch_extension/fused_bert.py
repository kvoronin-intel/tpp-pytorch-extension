...
class BertSelfAttention(BlockedModule):
    ...
    def __init__(self, config):
        ...
        assert self.position_embedding_type == "absolute", "self.position_embedding_type other than absolute not supported"
...
class BertEmbeddings(BlockedModule):
    ...
    def __init__(self, config):
        ...
        self.position_embedding_type = getattr(config, "position_embedding_type", "absolute")
        assert self.position_embedding_type == "absolute", f"position embedding type {self.position_embedding_type} not supported"
        ...
        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0: 
            print(f"config.hidden_size = {config.hidden_size}, config.intermediate_size = {config.intermediate_size}, p = {config.hidden_dropout_prob}, eps = {config.layer_norm_eps}")
...
try:
    ...
    def is_tensor(x):
        ...
        if transformers_orig_is_tensor(x): return True
        if isinstance(x, BlockedTensor): return True
        return False
