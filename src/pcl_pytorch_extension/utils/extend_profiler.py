...
def el_nested_key_averages(self, only_top_level):
    ...
    for evt in self:
        if only_top_level and getattr(evt, "cpu_parent", None): continue
...
def print_op_timings(prof, use_gpu=False, prefix='prof'):
    def get_interval(e):
        if hasattr(e, 'cpu_interval'): return e.cpu_interval
        #if hasattr(e, 'time_range'): return e.time_range
        return e.time_range
    sorted_fe = sorted(prof.function_events, key=lambda event: [get_interval(event).start, -get_interval(event).end],)
    ...
    with open("%s.OpTimings.txt" % prefix, "w") as f:
        for i, fe in enumerate(sorted_fe):
            ...
            cstr = ""
            if use_gpu:
               for kinfo in fe.kernels: cstr += " %10.3f %10.3f %8.3f %8.3f " % ((kinfo.interval.start - start_time) /1000.0, (kinfo.interval.end - start_time)/1000.0, (kinfo.interval.start - get_interval(fe).start) /1000.0, kinfo.interval.elapsed_us()/1000.0)
            print("%-6d %6d %12.4f %12.4f %12.4f %2s %s   %-40s    %s" % (i, fe.id, (get_interval(fe).start - start_time)/1000.0, (get_interval(fe).end - start_time)/1000.0, get_interval(fe).elapsed_us()/1000.0, fe.thread, cstr, fe_name.replace(' ', '_'), fe.input_shapes), file=f)
