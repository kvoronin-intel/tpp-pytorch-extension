...
class ReLU_bf16(Function):
    ...
    def backward(ctx, grad):  # Backward pass method
        r = ctx.saved_tensors
        mask = r[0]
        d_input = Conv1dOpti_cpp.relu_backward_bf16(
            grad.contiguous(), mask.contiguous()
        )
        return d_input
...
class Conv1dOptiFunction(Function):
    ...
    def forward(ctx, input, weight, dilation, enable_BF16):
        ...
        if enable_BF16 == False:  # Run the FP32 version
            ...
            result = Conv1dOpti_cpp.forward(input, weight, dilation[0])
        else:
    ...
    def backward(ctx, grad):
        ...
        if enable_BF16 == False:  # Run the FP32 version
            d_input, d_weight = Conv1dOpti_cpp.backward(
                grad.contiguous(), inp, weight, dilation[0]
            )
            ...
            if grad.dtype == torch.bfloat16:  # Check if gradiant is in BF16 format
                ...
                d_input, d_weight = Conv1dOpti_cpp.backward(
                    grad.to(torch.bfloat16).contiguous(), inp, weight, dilation[0]
                )
            return d_input, d_weight, None, None
...
class Conv1dOpti(_ConvNd):
    ...
    def forward(self, input):
        ...
        self.enable_BF16 = (
            ...
            and (
                input.size(2)
                >= (2 * (self.weight.size(2) - 1) * self.dilation[0] + 96 - 1)
            )
            and (input.size(2) % 2 == 0)
            and (self.weight.size(0) % 2 == 0)
            and (self.weight.size(1) % 2 == 0)
        )
        ...
        if self.padding_mode != "zeros":
            ...
            # else:
            return Conv1dOptiFunction.apply(
                input, self.weight, self.dilation, self.enable_BF16
            )
        ...
        if input.dtype != torch.bfloat16:  # Run the FP32 version
            ...
            return Conv1dOptiFunction.apply(
                input, self.weight, self.dilation, self.enable_BF16
            ) + self.bias.view([1, -1, 1]).to(torch.bfloat16)
